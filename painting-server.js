...
app.get('/f1/races', async (req, res) => {
    const {data, error} = await supabase
    .from('races')
    .select(`raceId, year, round, name, circuits (name,location,country)`).eq('raceId',req.params.race).order('positionOrder', { ascending: true }); 
    res.send(data);
});
...
app.get('/f1/results/:race', async (req, res) => {
    ...
    .from('results')
    .select(`
    raceId, resultId, positionOrder, races (year, name),
...
app.get('/f1/qualifying/:id', async (req, res) => {
    const {data, error} = await supabase
    .from('qualifying')
    .select(`
...
app.get('/f1/drivers/name/:prefix/limit/:count', async (req, res) => {
    const {data, error} = await supabase
    .from('drivers')
    .select(`
        ...
        nationality, url
    `)
    .ilike('surname', `${req.params.prefix}%`) // works just like SQL ilike keyword (where it lets you fitler out the start or end of a word)
